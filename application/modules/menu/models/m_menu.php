...
//                    throw new Exception('test');
	...
	public function getUserEditingInfo(){
				$sql="select count(*) as jumlah_artikel_tersunting from app_updateartikel_konten a where a.approvestatus_id=0";
				$params=array();
				$result = $this->mdb->QueryData('application', $sql, $params, 'record');
				$data_user_editing_info=array(""=>"");
				if (isset($result)) {
					$data_user_editing_info = $result[0];
				} else {
					$data_user_editing_info = array("info"=>"no data");
				}
					return $data_user_editing_info;	
